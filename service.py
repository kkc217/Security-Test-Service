...
#     x_train_int = convert_digest_to_int(x_train)
#     x_train_arr = np.array(x_train_int)
#     x_train_df = pd.DataFrame(x_train_arr[1:], columns=x_train_arr[0])
#     y_train_df = pd.DataFrame(y_train, columns=['Label'])
    
#     x_train_df = x_train_df.applymap(int)
#     y_train_df = y_train_df.applymap(int)
    
#     print(y_train_df)
    
#     model = xgb.XGBClassifier()
#     model.fit(x_train_df, y_train_df)
#     joblib.dump(model, './tlsh_xgb_model1.pkl')
#     print(model)
...
def make_pred(target, data_x, data_y) :
    ...
    load_model = joblib.load('./tlsh_xgb_model1.pkl')
...
def service(target_path) :
    origin_path = "F:/Malware/BIG2015/train/Label_tlsh.csv"
